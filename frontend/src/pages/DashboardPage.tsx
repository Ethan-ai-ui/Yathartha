...
export default function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const suspiciousCount = submissionsArray.filter(s => s.status === 'flagged' || s.misinformation_score! > 0.7).length;
      ...
      console.error(error);
      toast({ description: error instanceof Error ? error.message : 'Error loading dashboard data', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadReport = async () => {
    if (!tokens?.access) return toast({ description: 'Not authenticated', variant: 'destructive' });
    try {
      const response = await analyticsAPI.getReports(tokens.access);
      const blob = new Blob([JSON.stringify(response.results || response, null, 2)], { type: 'application/json' });
        ...
        <div className="flex justify-between items-start mb-8">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3">
              ...
              <Link to="/"><Button variant="ghost" size="sm" className="gap-2"><Home size={16} /> Home</Button></Link>
            </div>
        ...
        {analytics && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <Card>
              ...
              <CardContent><div className="text-3xl font-bold text-foreground">{analytics.total_submissions}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-green-400">Verified</CardTitle></CardHeader>
              <CardContent><div className="text-3xl font-bold text-green-400">{analytics.verified_count}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-red-400">Suspicious</CardTitle></CardHeader>
              <CardContent><div className="text-3xl font-bold text-red-400">{analytics.suspicious_count}</div></CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-yellow-400">Pending</CardTitle></CardHeader>
              <CardContent><div className="text-3xl font-bold text-yellow-400">{analytics.pending_count}</div></CardContent>
            </Card>
          </div>
        )}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="submissions" className="space-y-4">
            <Card>
              ...
              <CardContent>
                {loading ? (
                  <div className="flex justify-center py-8"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div></div>
                  ...
                  <div className="space-y-4">
                    {submissions.map((s) => (
                      <div key={s.id} className="border border-border rounded-lg p-4 hover:bg-muted/50 transition">
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex-1">
                            <p className="font-medium text-foreground">{s.content.substring(0, 100)}...</p>
                            <p className="text-sm text-muted-foreground mt-1">
                              Type: <span className="font-medium">{s.content_type}</span> • 
                              Status: <span className={`font-medium ${getStatusColor(s.status)}`}>{s.status}</span>
                            </p>
                            {s.misinformation_score !== undefined && (
                              <p className="text-sm text-muted-foreground mt-1">
                                Misinformation Score: <span className="font-bold">{(s.misinformation_score * 100).toFixed(1)}%</span>
                              </p>
                            )}
                            <p className="text-xs text-muted-foreground/70 mt-2">{new Date(s.created_at).toLocaleString()}</p>
                          </div>
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild><Button variant="ghost" size="sm"><MoreHorizontal size={16} /></Button></DropdownMenuTrigger>
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => handleShareSubmission(s.id)}><Share2 size={14} className="mr-2" /> Share</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => handleFlagSubmission(s.id)}><Flag size={14} className="mr-2" /> Flag</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </div>
                      </div>
          ...
          <TabsContent value="analytics" className="space-y-4">
            <Card>
              <CardHeader className="flex justify-between items-center">
                ...
                <Button onClick={handleDownloadReport} size="sm" className="gap-2" disabled={loading}><Download size={16} /> Download Report</Button>
              </CardHeader>
              <CardContent>
                {loading ? (
                  <div className="flex justify-center py-8"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div></div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                      ...
                      <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                          <Pie data={chartData} cx="50%" cy="50%" labelLine={false} label={({ name, value }) => `${name}: ${value}`} outerRadius={100} dataKey="value">
                    ...
                    <div className="flex flex-col justify-center space-y-4">
                      ...
                      <div className="p-4 bg-blue-500/10 rounded-lg border border-blue-500/20">
                        ...
                        <p className="text-2xl font-bold text-blue-400">
                          {analytics?.total_submissions ? (((analytics.verified_count + analytics.suspicious_count) / analytics.total_submissions) * 100).toFixed(1) : '0'}%
                        </p>
          ...
          <TabsContent value="settings" className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-foreground capitalize">{user?.role || 'Not specified'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Full Name</label>
                  <p className="mt-1 text-foreground">{user?.username || 'Not set'}</p>
                </div>
            ...
            <Card>
              ...
              <CardContent><Button variant="destructive" className="w-full">Delete Account</Button></CardContent>
            </Card>
          </TabsContent>
        </Tabs>
      </div>
